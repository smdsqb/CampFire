...
export default function CampList({ camps, activeCamp, onSelect }: Props) {
  const list = camps.length > 0 ? camps : SEED_CAMPS.map((c, i) => ({ ...c, id: String(i), createdAt: new Date(), createdBy: '' }))
  ...
  return (
    <div className="flex flex-col w-[200px] border-r border-[#2E2820] overflow-hidden flex-shrink-0 glass">
      ...
      <div className="px-3 pt-4 pb-3 border-b border-[#2E2820]">
        <div className="font-serif text-base font-semibold text-[#F5EFE8] flex items-center gap-1">
          🔥 CampFire
        </div>
      ...
      {/* My Camps */}
      <div className="px-3 pt-3 pb-1 text-[10px] font-semibold tracking-widest text-[#6B5A4A] uppercase">
        My Camps
      </div>
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0">
        <div className="text-xs font-medium truncate" style={{ color: active ? '#F97316' : '#F5EFE8' }}>
          {name}
        </div>
        {members && (
          <div className="text-[10px] text-[#6B5A4A]">{formatCount(members)} members</div>
        )}
      </div>
    </button>
  )
