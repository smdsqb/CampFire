...
export default function NewPostModal({ campId, campName, onClose }: Props) {
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-lg rounded-2xl border border-[#3D3228] p-6 flex flex-col gap-4"
        style={{ background: '#1A1410' }}
        onClick={e => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center justify-between">
          <div className="font-serif text-lg font-semibold text-[#F5EFE8]">
            🔥 Share with the camp
          </div>
          <button onClick={onClose} className="text-[#6B5A4A] hover:text-[#F5EFE8]">
            <X size={18} />
          </button>
        </div>
        ...
        {!campName && (
          <div>
            ...
            >
              {SEED_CAMPS.map(c => (
                <option key={c.name} value={c.name}>{c.icon} c/{c.name}</option>
              ))}
            </select>
          </div>
        )}
        ...
        <div className="flex justify-end gap-2">
          <button onClick={onClose} className="px-4 py-2 text-sm rounded-full text-[#A89880] border border-[#3D3228]"
